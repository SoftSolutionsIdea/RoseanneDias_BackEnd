...
export class ProductsService {
    ...
    async updateProducts(id: string, data: updateProductsDto) {
        ...
        const rental = await createOrUpdate(this.prisma.rental, { amount: data.rental.rental }, { amount: data.rental.rental });
        const category = await createOrUpdate(this.prisma.category, { time: data.category.category }, { time: data.category.category });
        const color = await createOrUpdate(this.prisma.color, { role: data.color.color }, { role: data.color.color });
        const image = await createOrUpdate(this.prisma.image, { cep: data.image.image }, { cep: data.image.image });
        const spentValue = await createOrUpdate(this.prisma.spentValue, { street: data.spentValue.spentValue }, { street: data.spentValue.spentValue });
        const status = await createOrUpdate(this.prisma.status, { city: data.status.status }, { city: data.status.status });
